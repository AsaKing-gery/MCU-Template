...
# 脚本在 tools/build/ 下，不能再靠一层 ".." 推算；找不到就退回当前目录
# （作为 VSCode 任务运行时，工作目录本来就是工程根）。
$root = $PSScriptRoot
if (-not $root) { $root = (Get-Location).Path }
while (-not (Test-Path -LiteralPath (Join-Path $root 'mcu.json'))) {
    $parent = Split-Path -Parent $root
    if (-not $parent -or $parent -eq $root) { $root = (Get-Location).Path; break }
    $root = $parent
}
